...
export async function getDataHygieneMetrics(filter: FilterOptions) {
  ...
  try {
    ...
    console.error("Data Hygiene Metrics error:", error);
    return {
      total_validated: 0,
      reachable_count: 0,
      reachable_rate: 0,
...
export async function getPilotMetrics(filterByTime: FilterOptions, filterByList: string) {
  ...
  try {
    ...
    return {
      sample_size: 0,
      target_connect_uplift_pct: 0,
      target_connect_rate: 0,
      success_connect_uplift_pct: 0,
      success_voicemail_uplift_pct: 0,
...
export async function getReattemptPotential(filter: FilterOptions) {
  ...
  try {
    ...
    console.log("Reattempt error: ", error);
    return {
      cooldown_contacts_count: 0,
      reattempt_potential: 0,
      target_kpi: 15,
